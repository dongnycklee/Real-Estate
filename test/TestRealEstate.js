...
contract('RealEstate', function(account) {
  ...
  it("가나슈 두번째 계정으로 매물 아이디 0번 매입 후 이벤트 생성 및 매입자 정보와 buyers 배열 테스팅", function(){
    return RealEstate.deployed().then(function(instance) {
      ...
      assert.equal(buyerInfo[0].toUpperCase(), accounts[1].toUpperCase(), "매입자의 계정이 가나슈 두번쨰 계정과 일치하지 않습니다.");
      ...
      assert.equal(buyers[0].toUpperCase(), accounts[1].toUpperCase(), "Buyers 배열 첫번째 인덱스 게정이 가나슈 두번쨰 계정과 일치하지 않습니다.");
    });
  });
});